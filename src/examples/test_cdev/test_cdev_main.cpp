...
ssize_t TestCdev::write(cdev::file_t *filp, const char *buffer, size_t buflen)
{
    ...
    PX4_INFO("Function : TestCdev::%s has been called!", __func__);
    // 打印写入的长度和内容，方便调试
    PX4_INFO("Write buflen is: %zu, and content is: %s", buflen, wbuf);
...
int test_cdev_main(int argc, char *argv[])
{
    ...
    TestCdev *cdev = new TestCdev(DEV_PATH);  // 创建驱动对象，传入设备路径
